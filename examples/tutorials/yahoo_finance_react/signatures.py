...
class AnalyzeStock(Signature):
    ...
    risk_factors: str = OutputField(desc="Key risk factors as JSON array: [\"risk1\", \"risk2\", \"risk3\"]")
    price_targets: str = OutputField(desc="Price targets as JSON object: {\"1_month\": 150.0, \"3_month\": 160.0, \"1_year\": 180.0}")
...
class CompareStocks(Signature):
    ...
    rankings: str = OutputField(desc="Stocks ranked as JSON array: [\"AAPL: Strong buy\", \"MSFT: Hold\", \"etc\"]")
...
class FinancialQuery(Signature):
    ...
    analysis_plan: str = OutputField(desc="Step-by-step plan as JSON array: [\"Step 1: Get stock price\", \"Step 2: Analyze trends\", \"etc\"]")
    required_data: str = OutputField(desc="Required data as JSON array: [\"current_price\", \"historical_data\", \"news\"]")
