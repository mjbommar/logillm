...
class MemoryAnalysis(Signature):
    ...
    extractable_facts: str = OutputField(description="List of facts worth remembering as JSON array: [\"fact1\", \"fact2\", \"fact3\"]")
    ...
    suggested_tags: str = OutputField(description="Tags for categorizing this memory as JSON array: [\"tag1\", \"tag2\", \"tag3\"]")
...
class PersonalizedResponse(Signature):
    ...
    relevant_memories: str = InputField(description="Memories relevant to this interaction as JSON array")
    ...
    action_items: str = OutputField(description="Any actions or reminders to set as JSON array: [\"action1\", \"action2\"]")
...
class ConversationPlanning(Signature):
    ...
    available_tools: str = InputField(description="Available memory and utility tools as JSON array")

    reasoning: str = OutputField(description="Step-by-step reasoning about how to respond")
    tool_calls: str = OutputField(description="Which tools to use and in what order as JSON array: [\"tool1\", \"tool2\"]")
