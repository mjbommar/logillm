...
class ClassifyEmail(Signature):
    ...
    email_type: str = OutputField(desc="Classify email type. Return ONLY ONE of these exact values: order_confirmation, support_request, meeting_invitation, newsletter, promotional, invoice, shipping_notification, urgent_alert, other")
    urgency: str = OutputField(desc="Classify urgency level. Return ONLY ONE of these exact values: low, medium, high, critical")
...
class ExtractEntities(Signature):
    ...
    important_dates: list[str] = OutputField(desc="List of important dates found. Format as JSON array: [\"2024-12-15\", \"2024-12-20\"]")
    contact_info: list[str] = OutputField(desc="Relevant contact information extracted. Format as JSON array: [\"support@example.com\", \"+1-555-123-4567\"]")
...
class GenerateActionItems(Signature):
    ...
    action_required: bool = OutputField(desc="Whether any action is required. Must be either true or false")
    action_items: list[str] = OutputField(desc="List of specific actions needed. Format as JSON array: [\"action1\", \"action2\"]")
