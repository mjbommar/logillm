            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt < max_attempts:
                        logger.warning(
                            f"Test {func.__name__} failed on attempt {attempt}: {e}"
                        )
                        import time
                        time.sleep(delay)
                    else:
...
def assert_llm_output_quality(
    ...
    if min_length is not None:
        assert len(output) >= min_length, (
            f"Output too short: {len(output)} < {min_length} chars"
        )
    
    if max_length is not None:
        assert len(output) <= max_length, (
            f"Output too long: {len(output)} > {max_length} chars"
        )
